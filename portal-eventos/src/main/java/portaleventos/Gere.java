...
public class Gere 
{
    ...
    //implementacao de métodos genéricos de CRUD (persist, merge, remove)
	//que funcionam para qualquer entidade (Pessoa, Pedido, Evento etc
	
	//<T> torna o método genérico, para aceitar qualquer tipo de entidade
    private <T> void persist(T entity) 
    {
        ...
        {
        	//a transação é necessária para garantir que as operações serao seguras
        	//se algo der errado, pode-se fazer o rollback (voltar atrás)
            tx = s.beginTransaction();
            s.persist(entity);//fazemos a operacao save para qualquer entidade, e insere um obj no BD, equivalente a um INSERT no SQL 
            tx.commit();
        } 
        catch (Exception e) 
        { 
    ...
    private <T> void merge(T entity) 
    {
        ...
        {
            tx = s.beginTransaction();
            s.merge(entity);//fazemos a operacao update para qualquer entidade
            tx.commit();
        } 
        catch (Exception e) 
        { 
    ...
    private <T> void remove(Class<T> clazz, Long id) 
    {
        ...
        {
            tx = s.beginTransaction();
            T obj = s.get(clazz, id); //busca o obj pelo ID na tabela correspondente (Pedido p = s.get(Pedido.class, 5))
            if (obj != null) s.remove(obj);//fazemos a operacao delete para qualquer entidade (remove(Pedido.class, 5); apaga o pedido com id = 5
            tx.commit();
        } 
        catch (Exception e) 
        { 
 ...
 public List<Pessoa> listarPessoas() 
 {
     try (Session s = HibernateUtil.getSessionFactory().openSession()) 
     {
    	 //O método createQuery() cria uma consulta (query) em HQL (Hibernate Query Language),
    	 //que é parecida com o SQL, mas baseada em nomes de classes e atributos, não em tabelas e colunas.
    	 
    	 //from Pessoa significa: Seleciona todas as entidades Pessoa do banco de dados.”
    	 //O Hibernate converte isso internamente em:
         //SELECT * FROM pessoa;
    	 //Pessoa.class diz ao hibernate que tipo de obj deve retornar, obj da classe pessoa, e ao chamar .list, recebemos list<Pessoa>
 ...
 public List<Area> listarAreasPorDepartamento(Long depId)
 {
     try (Session s = HibernateUtil.getSessionFactory().openSession()) 
     {
         ...
         //SELECT * FROM area WHERE departamento_id = ?;
         //retorna uma lista de objetos Area pertencentes a um determinado departamento.
     }
 }
 ...
 public List<TipoEvento> listarTiposEvento() 
 {
     try (Session s = HibernateUtil.getSessionFactory().openSession()) 
     {
    	 return s.createQuery("from portaleventos.TipoEvento", TipoEvento.class).list();
     }
 }
 ...
 public List<Pedido> listarPedidosPorEstado(PedidoEstado estado) 
 {
     try (Session s = HibernateUtil.getSessionFactory().openSession()) 
     {
         ...
         //SELECT * FROM pedido WHERE estado = 'APROVADO' ORDER BY data_submissao DESC; do mais recente ao antigo
     }
 }
 ...
 public Evento publicarEventoAPartirDoFormulario(Long pedidoId) {
	    ...
	    try (Session s = HibernateUtil.getSessionFactory().openSession()) {
	        ...
	        //Carregar o Formulário com sessão aberta
	        FormularioEvento f = s.createQuery(
	                "from FormularioEvento f where f.pedido.id = :pid", FormularioEvento.class)
	                .setParameter("pid", pedidoId)
	                .uniqueResult();

	        if (f == null)
	            throw new IllegalArgumentException("Formulário não encontrado para o pedido " + pedidoId);

	        //carregar enquanto a sessão ainda está aberta 
	        //Previne o erro “failed to lazily initialize a collection of role” porque a sessao fechava antes de pegarmos as colecoes
	        Hibernate.initialize(f.getOradores());
	        Hibernate.initialize(f.getPatrocinadores());
	        Hibernate.initialize(f.getInstrutores());
	        Hibernate.initialize(f.getArtistas());
	        Hibernate.initialize(f.getObras());

	        //Criar o Evento com base no Formulário
	        Evento e = new Evento();
	        ...
	        if ("Festa".equalsIgnoreCase(tipo)) {
	            ...
	            if (Boolean.TRUE.equals(f.getInscricaoPrevia()) &&
	                (f.getVagasPraticas() == null || f.getVagasPraticas() <= 0))
 ...
 public void cancelarInscricao(Long inscricaoId) 
 {
     try (Session s = HibernateUtil.getSessionFactory().openSession()) 
     {
         ...
         //buscar o obj pedo id de inscricao na tabela correspondente
